...
<?php require_once "template-parts/pc_quick_access.php"; ?>
<div class="row">
        <div class="col-md-12 text-center">
            <h3 class="animated pulse infinite text-info fw-bolder text-uppercase">Computer Inventory</h3>
        </div>
    </div>
...
$dateService = new dateAcquiredService($conn);
$dateCategory = $dateService->getdateService();
...
<small class="text-primary fw-bold">By Date Acquired</small>
 <select id="filter" class="form-control resetSearch me-1 mb-md-0 mb-2">
 	<option value=""></option>
 	<?php  foreach ($dateCategory as $dateService) { ?>
 	<option value="<?= $dateService; ?>"><?= $dateService; ?></option>
 	<?php } ?>
 </select>
</div>
...
<div class="col-md-2">
		<?php
			$yrService = new YearsofService($conn);
			$yrsService = $yrService->yearofService();
		?>
		<small class="text-primary fw-bold">By Years of Service</small>
		<select id="filterYrService" class="form-control resetSearch me-1 mb-md-0 mb-2">
		<option value=""></option>
		<?php foreach ($yrsService as $yrServices) { ?>
		<option value="<?= $yrServices; ?>"><?= $yrServices; ?></option>
		<?php } ?>
		</select>
</div>
...
<div class="col-md-3">
		...
		<input type="text" id="filterType" class="form-control resetSearch">
</div>

<div class="col-md-1 text-md-center">
	<a href="pc_inventory" type="button" class="btn btn-outline-danger btn-sm mt-md-4 mt-3 d-md-block d-none">Reset</a>
		<a href="pc_inventory" type="button" class="btn btn-outline-danger btn-sm mt-md-4 mt-3 d-md-none">Reset</a>
</div>
...
<a href="#" type="button" class="btn btn-sm btn-outline-success edit-dataId" data-bs-toggle="modal" data-bs-target="#modalUpdate" id="<?= $row['id']; ?>"><i class="fa fa-underline"></i></a> <a href="#" type="button" class="btn btn-sm btn-outline-danger delete-dataId" data-bs-toggle="modal" data-bs-target="#modalDelete" id="<?= $row['id']; ?>"><i class="fa fa-eraser"></i></a>
</td>
</tr>
<?php
$ctr++;
...
$totalRecords = $recordsManager->getTotalRecordsCount(); //method to get the total number of records

$pageCount = ceil($totalRecords / $recordsPerPage); //calculate total number pages


$rec = $recordsManager->getRecords($page, $recordsPerPage);
...
<script type="text/javascript" src="js/print_pc_inventory.js"></script>

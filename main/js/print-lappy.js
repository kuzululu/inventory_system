...
// laptop aging print aging records
let printAgingLaptopRecords = () => {
// Make an AJAX request to fetch aging records from PHP
let xhr = new XMLHttpRequest();
xhr.open("GET", "Apps/fetch_laptop.php", true);
...
if (xhr.status === 200) {
 try {
     ...
     let totalLappyAging = responseData.total_aging; 
    
     // Format the records for printing
     let printArea = "<h2 class='text-center text-success'>Laptop Aging Records</h2>";
      printArea += "<h5 class='fw-bolder'>Total Aging Count: <span class='fw-bolder text-success'>"+totalLappyAging+"</span></h5>";
     printArea += "<div class='table-responsive'>";
     printArea += "<table class='table table-hover'>";
     printArea += "<thead><tr><th>No.</th> <th>Services</th> <th>Tag Name</th> <th>Description</th> <th>Property Tag</th> <th>Date Acquired</th> <th>Years Services</th> <th>Actual User</th> <th>Remarks</th> <th>Status</th></tr></thead>";
     let counter = 1;
...
// trigger buttons
let btnprintLaptopAging = document.querySelector("#printLappyAging");
btnprintLaptopAging.addEventListener("click", printAgingLaptopRecords, false);

...
require_once "modal/laptopModal.php";
?>

<section id="pc-inventory" class="mt-5 pt-2">

<div class="container-fluid mb-3 mt-3">
<?php require_once "template-parts/laptop_quick_access.php"; ?>
<div class="row">
        <div class="col-md-12 text-center">
            <h3 class="animated pulse infinite text-info fw-bolder text-uppercase">Laptop Inventory</h3>
        </div>
    </div>
...
$dateService_laptop = new dateAcquiredService($conn);
$dateCategory_laptop = $dateService_laptop->getdateLaptopService();
...
<small class="text-primary fw-bold">By Date Acquired</small>
 <select id="filterLaptop" class="form-control resetSearch me-1 mb-md-0 mb-2">
 	<option value=""></option>
 	<?php  foreach ($dateCategory_laptop as $dateService_laptop) { ?>
 	<option value="<?= $dateService_laptop; ?>"><?= $dateService_laptop; ?></option>
 	<?php } ?>
 </select>
</div>
...
<div class="col-md-2">
		<?php
			$yrService_laptop = new YearsofService($conn);
			$yrsService_laptop = $yrService_laptop->yearofLaptopService();
		?>
		<small class="text-primary fw-bold">By Years of Service</small>
		<select id="filterLaptopYrService" class="form-control resetSearch me-1 mb-md-0 mb-2">
		<option value=""></option>
		<?php foreach ($yrsService_laptop as $yrServices_laptop) { ?>
		<option value="<?= $yrServices_laptop; ?>"><?= $yrServices_laptop; ?></option>
		<?php } ?>
		</select>
</div>
...
<div class="col-md-3">
		...
		<input type="text" id="filterLaptopType" class="form-control resetSearch">
</div>

<div class="col-md-1 text-md-center">
	<a href="laptop_inventory" type="button" class="btn btn-outline-danger btn-sm mt-md-4 mt-3 d-md-block d-none">Reset</a>
		<a href="laptop_inventory" type="button" class="btn btn-outline-danger btn-sm mt-md-4 mt-3 d-md-none">Reset</a>
</div>
...
private $recordsLaptop;

public function __construct($recordsLaptop){
$this->recordsLaptop = $recordsLaptop;
}	

public function displayRecords(){
	$ctr = 1;
while ($row = $this->recordsLaptop->fetch_assoc()) {
...
<a href="#" type="button" class="btn btn-sm btn-outline-success editLaptopId" data-bs-toggle="modal" data-bs-target="#modalUpdate" id="<?= $row['id']; ?>"><i class="fa fa-underline"></i></a> <a href="#" type="button" class="btn btn-sm btn-outline-danger delLaptopId" data-bs-toggle="modal" data-bs-target="#modalDelete" id="<?= $row['id']; ?>"><i class="fa fa-eraser"></i></a>
</td>
</tr>
<?php
$ctr++;
...
$totalRecords = $recordsManager->getLaptopTotalRecordsCount(); //method to get the total number of records

$pageCount = ceil($totalRecords / $recordsPerPage); //calculate total number pages


$rec = $recordsManager->getLaptopRecords($page, $recordsPerPage);
...
<script type="text/javascript" src="js/print_laptop_inventory.js"></script>

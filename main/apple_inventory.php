...
require_once "modal/appleModal.php";
?>
...
<div class="row">
        <div class="col-md-12 text-center">
            <h3 class="animated pulse infinite text-info fw-bolder text-uppercase">Apple Inventory</h3>
        </div>
    </div>
...
$dateService_apple = new dateAcquiredService($conn);
$dateCat_apple = $dateService_apple->getdateAppleService();
...
<small class="text-primary fw-bold">By Date Acquired</small>
 <select id="filterApple" class="form-control resetSearch me-1 mb-md-0 mb-2">
 	<option value=""></option>
 	<?php  foreach ($dateCat_apple as $dateService_apple) { ?>
 	<option value="<?= $dateService_apple; ?>"><?= $dateService_apple; ?></option>
 	<?php } ?>
 </select>
</div>
...
<div class="col-md-2">
		<?php
			$yrService_apple = new YearsofService($conn);
			$yrsService_apple = $yrService_apple->yearofAppleService();
		?>
		<small class="text-primary fw-bold">By Years of Service</small>
		<select id="filterAppleYrService" class="form-control resetSearch me-1 mb-md-0 mb-2">
		<option value=""></option>
		<?php foreach ($yrsService_apple as $yrServices_apple) { ?>
		<option value="<?= $yrServices_apple; ?>"><?= $yrServices_apple; ?></option>
		<?php } ?>
		</select>
</div>
...
<div class="col-md-3">
		...
		<input type="text" id="filterAppleType" class="form-control resetSearch">
</div>

<div class="col-md-1 text-md-center">
	<a href="apple_inventory" type="button" class="btn btn-outline-danger btn-sm mt-md-4 mt-3 d-md-block d-none">Reset</a>
		<a href="apple_inventory" type="button" class="btn btn-outline-danger btn-sm mt-md-4 mt-3 d-md-none">Reset</a>
</div>
...
private $recordsApple;

public function __construct($recordsApple){
$this->recordsApple = $recordsApple;
}	

public function displayRecords(){
	$ctr = 1;
while ($row = $this->recordsApple->fetch_assoc()) {
...
<a href="#" type="button" class="btn btn-sm btn-outline-success editAppleId" data-bs-toggle="modal" data-bs-target="#modalUpdate" id="<?= $row['id']; ?>"><i class="fa fa-underline"></i></a> <a href="#" type="button" class="btn btn-sm btn-outline-danger delAppleId" data-bs-toggle="modal" data-bs-target="#modalDelete" id="<?= $row['id']; ?>"><i class="fa fa-eraser"></i></a>
</td>
</tr>
<?php
$ctr++;
...
$totalRecords = $recordsManager->getAppleTotalRecordsCount(); //method to get the total number of records

$pageCount = ceil($totalRecords / $recordsPerPage); //calculate total number pages


$rec = $recordsManager->getAppleRecords($page, $recordsPerPage);
...
<script type="text/javascript" src="js/print_apple_inventory.js"></script>
